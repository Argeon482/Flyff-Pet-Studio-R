            ...
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm">
                ...
                <div className="mb-4">
                    ...
                    <input 
                        ...
                        className={`mt-1 w-full bg-gray-700 text-white rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 ${error ? 'ring-red-500' : 'focus:ring-cyan-500'}`}
...
const NpcExpirationDisplay: React.FC<{ expiration: string | null; simulatedTime: number | null; remainingDurationMs?: number }> = ({ expiration, simulatedTime, remainingDurationMs }) => {
    ...
    if (remaining <= 0) return <span className="text-sm text-red-400 font-bold">Expired</span>;
...
const FactoryFloor: React.FC<FactoryFloorProps> = ({ 
    ...
    const handleNpcChange = (houseId: number, slotIndex: number, field: keyof House['slots'][0]['npc'], value: any) => {
        ...
        if (house) {
            ...
            if (field === 'type' && !value) {
                ...
                delete npcSlot.remainingDurationMs;
            }
    ...
    const handleExpirationSave = (remainingMs: number) => {
        ...
        if (house) {
            ...
            const oldExpiration = newSlots[slotIndex].npc.expiration ? new Date(newSlots[slotIndex].npc.expiration).getTime() : null;
            const expirationDate = new Date(now + remainingMs);
            
            newSlots[slotIndex].npc.expiration = expirationDate.toISOString();
            delete newSlots[slotIndex].npc.remainingDurationMs;

            // If pet timer was active but paused due to expiration, shift it forward
            const pet = newSlots[slotIndex].pet;
            if (oldExpiration && pet.finishTime && pet.startTime) {
                if (oldExpiration < now) {
                     const pausedDuration = now - oldExpiration;
                     // Shift pet times forward by the time it was expired
                     if (pausedDuration > 0) {
                         pet.finishTime += pausedDuration;
                         pet.startTime += pausedDuration;
                     }
                }
            }
    ...
    const handleConfirmAction = () => {
        ...
        switch (confirmAction.type) {
            case 'clear_houses': handleClearAllHouses(); break;
            case 'reset': if (confirmAction.houseId) handleResetHouse(confirmAction.houseId); break;
            case 'remove': if (confirmAction.houseId) onRemoveHouse(confirmAction.houseId); break;
        }
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-end items-center gap-4">
                 <button onClick={onOpenVirtualHouseModal} className="bg-purple-700 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded flex items-center gap-2">
                    <VirtualIcon /> Manage Virtual Houses
                </button>
                 <button onClick={() => setIsAddHouseModalOpen(true)} className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded">
                    Add House
                </button>
                 <button onClick={() => setConfirmAction({ type: 'clear_houses', title: 'Confirm Clear All Houses', message: <p>Are you sure you want to <strong>PERMANENTLY REMOVE ALL</strong> houses?</p> })} className="bg-red-800 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                    Clear All Houses
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-700">
                        <thead className="bg-gray-700/50">
                            <tr>{['House', 'Division', 'Service Block', 'Slot 1', 'Slot 2', 'Slot 3', 'Actions'].map(h => <th key={h} className="px-3 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">{h}</th>)}</tr>
                        </thead>
                                ...
                                <tr key={house.id}>
                                    <td className="px-3 py-4 align-top">
                                        <div className="flex flex-col gap-1">
                                            {isEditingLabel ? (
                                                <input type="text" value={editLabelValue} onChange={e => setEditLabelValue(e.target.value)} onBlur={() => handleLabelSave(house.id)} onKeyDown={e => e.key === 'Enter' && handleLabelSave(house.id)} className="w-28 bg-gray-600 text-white rounded px-1 py-0.5 text-sm" autoFocus />
                                            ) : (
                                                <span onClick={() => { setEditingLabelId(house.id); setEditLabelValue(house.label || `House #${house.id}`); }} className="text-sm font-bold text-gray-200 cursor-pointer hover:text-cyan-400 border-b border-transparent hover:border-cyan-400 transition-colors" title="Click to rename">{house.label || `House #${house.id}`}</span>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-3 py-4">
                                        <select value={house.division} onChange={e => handleFieldChange(house.id, 'division', e.target.value as Division)} className="w-32 bg-gray-700 text-white rounded p-1">{DIVISIONS.map(div => <option key={div} value={div}>{div}</option>)}</select>
                                    </td>
                                    ...
                                    {house.slots.map((slot, i) => {
                                        ...
                                        const virtualHouse = slot.npc.virtualHouseId ? virtualHouses.find(vh => vh.id === slot.npc.virtualHouseId) : null;
                                        
                                        // Expired Logic
                                        const isExpired = slot.npc.expiration && new Date(slot.npc.expiration).getTime() <= now;

                                        return (
                                        <td key={i} className="px-3 py-4 align-top relative">
                                            {slot.npc.type && (
                                                 <button onClick={() => handleNpcChange(house.id, i, 'mode', slot.npc.mode === 'LINKED' ? 'SOLO' : 'LINKED')} className="absolute top-1 right-1 p-1 rounded hover:bg-gray-600" title={slot.npc.mode === 'LINKED' ? 'Mode: Linked' : 'Mode: Solo'}>
                                            ...
                                            <div className="flex flex-col gap-2 w-40">
                                                ...
                                                {virtualHouse && slot.npc.mode === 'SOLO' && <div className="text-xs bg-purple-900 text-purple-200 px-1.5 py-0.5 rounded border border-purple-700 truncate"><span className="font-bold">VH:</span> {virtualHouse.name}</div>}
                                                {slot.npc.type && <>
                                                    ...
                                                    <div className="bg-gray-700 rounded p-1 flex items-center justify-between">
                                                        <NpcExpirationDisplay expiration={slot.npc.expiration} simulatedTime={simulatedTime} remainingDurationMs={slot.npc.remainingDurationMs} />
                                                    ...
                                                    <div>
                                                        {isExpired && slot.pet.startTime ? (
                                                            <span className="text-red-500 font-bold text-xs animate-pulse bg-red-900/20 px-2 py-1 rounded border border-red-800 block text-center">PAUSED (NPC Expired)</span>
                                                        ) : (
                                                            ...
                                                            : slot.pet.finishTime 
                                                                ...
                                                                : <div className="flex items-center gap-1.5">
                                                                    <button onClick={() => handleStartPetClick(house.id, i)} className={`${isConfirmingStart ? 'bg-yellow-600 hover:bg-yellow-500' : 'bg-green-700 hover:bg-green-600'} text-white text-xs font-bold py-1 px-2 rounded`}>{isConfirmingStart ? 'Confirm?' : 'Start'}</button>
                                                                    <button onClick={() => { const cycle = cycleTimes.find(c => c.npcType === slot.npc.type); if (cycle) setProgressInputSlot({ houseId: house.id, slotIndex: i, totalHours: cycle.time }); }} className="bg-blue-700 hover:bg-blue-600 text-white text-xs font-bold py-1 px-2 rounded">by %</button>
                                                                  </div>
                                                        )}
                                                    </div>
                                                </>}
                                    ...
                                    <td className="px-3 py-4 align-top">
                                        <div className="flex flex-col gap-1.5">
                                            <button onClick={() => setConfirmAction({ type: 'reset', houseId: house.id, title: 'Confirm Reset', message: <p>Are you sure?</p> })} className="bg-yellow-700 hover:bg-yellow-600 text-white font-bold py-1 px-2 rounded text-xs">Reset</button>
                                            <button onClick={() => setConfirmAction({ type: 'remove', houseId: house.id, title: 'Confirm Removal', message: <p>Are you sure?</p> })} className="bg-red-800 hover:bg-red-700 text-white font-bold py-1 px-2 rounded text-xs">Remove</button>
                                        </div>
                                    </td>
                                </tr>
                            )})}
            ...
            {progressInputSlot && <SetProgressModal onClose={() => setProgressInputSlot(null)} onSave={handleProgressSave} totalHours={progressInputSlot.totalHours} />}
            {expirationInputSlot && <SetExpirationModal onClose={() => setExpirationInputSlot(null)} onSave={handleExpirationSave} />}
            <ConfirmationModal isOpen={!!confirmAction} onClose={() => setConfirmAction(null)} onConfirm={handleConfirmAction} title={confirmAction?.title || ''}>{confirmAction?.message}</ConfirmationModal>
            <AddHouseModal isOpen={isAddHouseModalOpen} onClose={() => setIsAddHouseModalOpen(false)} onAddHouses={onAddHousesFromTemplate} />
        </div>
    );
};
